...
class CarliniL2:
	def __init__(self, sess, model, mean, batch_size=1, confidence=CONFIDENCE,
		...
		self.tlab = tf.Variable(
			np.zeros((batch_size, num_labels)), dtype=tf.float32)
		...
		self.l2dist = tf.reduce_sum(
			tf.square(self.newimg - (tf.tanh(self.timg) / 2 - mean + 0.5)), [1])
		...
		# TODO: We change start_vars to end_vars
		self.init = tf.variables_initializer(var_list=[modifier] + end_vars)

	def attack(self, imgs, targets, param_values):
		"""
		...
		for i in range(0, len(imgs), self.batch_size):
			...
			r.extend(self.attack_batch(
				imgs[i:i + self.batch_size], targets[i:i + self.batch_size], param_values))
		return np.array(r)

	def attack_batch(self, imgs, labs, param_values):
		"""
