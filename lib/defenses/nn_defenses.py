...
def retrain_defense(model_dict, data_dict, input_var, target_var,
                    test_prediction, dev_list, adv_x_all, rd, X_train, y_train,
    ...
    recons_flag = 0
    # TODO: Fixing batchsize
    batchsize = 500
    test_len = data_dict['test_len']
    n_features = data_dict['no_of_features']
    dim_red = model_dict['dim_red']

    # Doing dimension reduction on the dataset
    print("Doing {} with rd={} over the dataset".format(dim_red, rd))
    ...
    # Create new data_dict based on dimension reduced data
    data_dict_dr = get_data_shape(X_train_dr, X_test_dr, X_val_dr)

    # Create new Theano variables for input and output of model
    input_var = T.tensor3('inputs')
    target_var = T.ivector('targets')
    # Create/load model
    network, model_exist_flag = model_creator(model_dict, data_dict_dr,
                                              input_var, target_var, rd=rd)
